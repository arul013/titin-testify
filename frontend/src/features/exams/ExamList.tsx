...
const STATUS_META: Record<
  ExamStatus,
  { label: string; variant: 'success' | 'info' | 'neutral' | 'warning'; dot: string }
> = {
  ...
  >
    <span
      className={cn(
        ...
        danger ? 'bg-red-50 text-red-500' : 'bg-slate-100 text-slate-500 group-hover:bg-brand/10 group-hover:text-brand',
      )}
    >
      {icon}
    </span>
    ...
    <>
      <div className="flex flex-col gap-3">
        {exams.map((exam) => {
          ...
          const rowCanEdit = exam.status === 'draft' || exam.status === 'published';
              ...
              <div className="flex min-w-0 flex-1 items-start gap-3.5">
                ...
                <div className="flex min-w-0 flex-col gap-2">
                  <div className="flex flex-wrap items-center gap-2.5">
                    <h3 className="truncate text-base font-extrabold text-slate-800">{exam.title}</h3>
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs">
                    ...
                    <Meta icon={<Clock className="h-3.5 w-3.5" />}>{exam.duration_minutes} menit</Meta>
                    <Meta icon={<ClipboardList className="h-3.5 w-3.5" />}>
                      {exam.total_target} soal · {exam.sections.length} bagian
                    </Meta>
                    {exam.passing_value != null && (
                      <Meta icon={<Target className="h-3.5 w-3.5" />}>Lulus ≥ {exam.passing_value}</Meta>
                    )}
                    <Meta icon={<Users className="h-3.5 w-3.5" />}>
                      {exam.participants_count} peserta
                      {rowHasAttempts && (
                        <span className="font-semibold text-emerald-600"> · {exam.attempts_count} mengerjakan</span>
                      )}
                    </Meta>
                  </div>
                  {isSuperAdmin && (
                    <span className="text-[11px] text-slate-400">oleh {exam.creator_name || 'Super Admin'}</span>
                  )}
                </div>
              </div>
