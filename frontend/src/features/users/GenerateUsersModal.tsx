...
export const GenerateUsersModal: React.FC<GenerateUsersModalProps> = ({
  ...
  const handleDownloadCSV = () => {
    ...
    const rows = list.map((u) => `"${u.full_name}","${u.username}","${u.password}"`).join('\r\n');
      ...
      {!list ? (
        <form onSubmit={onSubmit} className="flex flex-col gap-4">
          <p className="text-sm text-gray-500 leading-relaxed">
            Masukkan nama lengkap peserta ujian di bawah ini. Username dan password akan secara otomatis
            dibuat untuk masing-masing peserta.
          </p>
          ...
          <div className="flex justify-end gap-3 border-t border-slate-150 pt-4 mt-2">
            <Button variant="ghost" type="button" onClick={onClose} disabled={isGenerating} className="font-bold">
              Batal
            </Button>
            <Button variant="primary" type="submit" loading={isGenerating} className="font-bold">
              Mulai Generate
            </Button>
          </div>
        </form>
          ...
          <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl text-emerald-800 text-sm flex gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-emerald-600 leading-relaxed">
                Berhasil membuat <strong>{list.length}</strong> akun peserta baru. Silakan unduh daftar
                kredensial berikut untuk dibagikan.
              </p>
            </div>
          </div>
