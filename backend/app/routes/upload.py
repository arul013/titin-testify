...
async def upload_image(
    ...
    """Upload gambar soal/passage (mis. diagram) ke Cloudflare R2.

    Akses dibatasi untuk Admin & Super Admin.
    """
    # 1. Validate file type
    if not file.content_type or not file.content_type.startswith("image/"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="File yang diunggah harus berformat gambar (jpg, png, webp, dsb).",
        )
    ...
    if not all([
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Penyimpanan gambar belum dikonfigurasi di backend.",
        )

    # 3. Generate unique filename
    original_name = file.filename or ""
    file_ext = original_name.rsplit(".", 1)[-1] if "." in original_name else "png"
    unique_filename = f"{uuid.uuid4().hex}.{file_ext}"
    r2_key = f"images/{unique_filename}"
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Gagal menginisialisasi penyimpanan gambar: {str(e)}",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Gagal mengunggah gambar ke penyimpanan: {str(e)}",
        )
    finally:
        await file.close()
