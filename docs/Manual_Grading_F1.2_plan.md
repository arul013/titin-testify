...
- **F1.2.1 — Grading + agregasi + gating ✅** (2026-08-01, belum commit):
  - Backend: `models/grading.py` + `services/grading_service.py` (authz owner/super_admin; `list_pending` ujian ber-antrean, `list_exam_attempts` attempt pending + progress dinilai, `get_attempt_detail` item manual + teks peserta + kriteria rubrik beku, `submit_grade` validasi skor/kriteria → simpan awarded/rubric_scores/feedback/graded_by/at → `_recompute_and_maybe_complete`: bila semua item manual dijawab ter-nilai → skor final poin `Σawarded/Σmax×100` + passed + `grading_status='complete'`) + `routes/grading.py` (`/api/grading/*`, require_admin). `review_attempt` + `AttemptReviewQuestion` diperluas (scoring_mode/awarded_score/max_score/rubric_scores/feedback). `MyExamItem`+`AttemptResultResponse` bawa `grading_status`.
  - Frontend: menu sidebar **Penilaian**; `features/grading/` (useGrading hooks + `PenilaianPage` daftar ujian + `GradingAttemptsPage` daftar peserta + `GradingDetailPage` UI grader: SoalPanel + teks peserta + input skor/kriteria + feedback + simpan→recompute) + 3 route tipis. Gating: `AttemptResultPage` hero "Menunggu Penilaian" saat pending; `RiwayatPage` badge+skor "Menunggu Penilaian"; `AttemptReviewPanel` esai tampilkan skor+rubrik+feedback setelah complete.
  - tsc/import/diagnostics bersih. **F1.2 LENGKAP end-to-end** (buat rubrik→soal esai→ujian→kerjakan→submit(pending)→nilai→complete→hasil+pembahasan).
