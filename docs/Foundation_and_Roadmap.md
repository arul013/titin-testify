...
Urutan default: **M1 → (F3/F4 secukupnya) → M2 → M3 → M4 → M5 → M6 → M7 → M8 → M9**. F1/F2 sudah disuntik sebagai sprint fondasi. **Selesai s/d M6 Fase 1** (M3 & reminder M6 tinggal jadwalkan cron di deploy). **Kandidat berikutnya: M6 Fase 2 (email/Resend), M7 (pra-ujian+ketahanan+aksesibilitas), atau M8 (anti-cheat perilaku).**
