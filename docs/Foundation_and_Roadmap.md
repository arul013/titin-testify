...
| **M5** | **Manajemen peserta**: assign per **grup/kelas** (cohort), **perpanjangan waktu per-peserta** (akomodasi), re-invite. | — | 🟡 **M5.1 grup + M5.2 akomodasi SELESAI** (2026-08-04, migrasi 024+025 nunggu user). **M5.1**: tabel `participant_groups`+`participant_group_members` (RLS lockdown), CRUD `/api/participant-groups`, UI `GroupManagerModal` di StepParticipants. **M5.2**: `exam_participants.extra_minutes` (mig. 025); timer akomodasi — durasi personal +extra & dinding `ends_at` **digeser +extra** utk peserta ybs (mode global; per-bagian: limit disebar prorata via `_distribute_extra`); diterapkan di `start_attempt` + `expire_stale_attempts`; `update_exam` participant kini **diff** (bukan replace-all) agar extra tak hilang; endpoint `PATCH /api/exams/{id}/participants/{uid}/extra-time` (audit); UI `AccommodationModal` dari menu Kelola. **M5.3** re-invite/reset menyusul. |
