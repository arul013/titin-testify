...
| **M7** | **Layar pra-ujian** (instruksi, durasi, jumlah soal, **pakta integritas**) + **ketahanan koneksi** (indikator tersimpan, auto-resume) + **aksesibilitas** (ukuran font/kontras). | — | 🟡 **M7.1 pra-ujian SELESAI** (2026-08-05, tanpa migrasi): gerbang `PreExamGate` sebelum attempt/timer — info (durasi/jumlah soal/bagian/jadwal/mode) + instruksi (dari `description`) + **pakta integritas** (checkbox) → "Mulai Ujian" baru memanggil `start()`; resume (ada attempt in_progress) lewati pakta. Backend `GET /api/my-exams/{id}/intro` (meta tanpa memulai). **M7.2 ketahanan koneksi** (indikator simpan + retry + banner offline) & **M7.3 aksesibilitas** (skala font + kontras tinggi) menyusul. |
...
Urutan default: **M1 → (F3/F4 secukupnya) → M2 → M3 → M4 → M5 → M6 → M7 → M8 → M9**. F1/F2 sudah disuntik sebagai sprint fondasi. **Selesai s/d M6 Fase 1 + M7.1** (M3 & reminder M6 tinggal jadwalkan cron di deploy). **Kandidat berikutnya: M7.2 (ketahanan koneksi) & M7.3 (aksesibilitas), M6 Fase 2 (email/Resend), atau M8 (anti-cheat perilaku).**
