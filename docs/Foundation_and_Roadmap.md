...
- **Extensible by data, not by rewrite**: perbedaan antar jenis tes diakomodasi lewat _data/konfigurasi_, bukan cabang kode/relasi baru tiap kali.
...
- **F3 (auditability)** ✅ infra — `audit_events` append-only, soft-delete, optimistic concurrency (migrasi 016). _Catatan: emit audit di setiap aksi mutasi masih tumbuh seiring fitur (M1 sudah emit)._
...
- **F1 (model soal ekstensibel)** ✅ inti — lihat rincian sub-fase di bawah. Ini juga **merealisasikan M9** (tipe non-MCQ + grading).
  ...
  - **F1.3** speaking (jawaban audio) ⏸️ — blocker storage **TERATASI** (2026-08-06: `storage_service` + `STORAGE_BACKEND=supabase`); tinggal endpoint upload audio peserta + UI rekam.
...
| **M5**    | **Manajemen peserta**: assign per **grup/kelas** (cohort), **perpanjangan waktu per-peserta** (akomodasi), re-invite. | —          | ✅ **SELESAI** (2026-08-04/05, migrasi 024+025+026 nunggu user). **M5.1**: tabel `participant_groups`+`participant_group_members` (RLS lockdown), CRUD `/api/participant-groups`, UI `GroupManagerModal` di StepParticipants. **M5.2**: `exam_participants.extra_minutes` (mig. 025); timer akomodasi — durasi personal +extra & dinding `ends_at` **digeser +extra** (global; per-bagian prorata via `_distribute_extra`); di `start_attempt`+`expire_stale_attempts`; `update_exam` participant kini **diff**; `PATCH .../participants/{uid}/extra-time` + `AccommodationModal`. **M5.3 re-invite/reset**: `exam_attempts.reset_at`/`reset_by` (mig. 026) — void LUNAK (auditable, bukan hapus); diabaikan di `start_attempt`/`list_my_exams`/`expire` & dikecualikan dari hasil/analitik; `POST /api/admin/attempts/{id}/reset` (audit `attempt.reset`); tombol Reset per-baris di halaman Hasil + ConfirmDialog. |
| **M6**    | **Notifikasi** (ujian ditugaskan / akan dibuka / pengingat) — in-app + email.                                         | F4 (job)   | 🟡 **Fase 1 (in-app) SELESAI** (2026-08-05, migrasi 027 nunggu user): tabel `notifications` (RLS lockdown, idempoten `UNIQUE(user_id,type,entity_id)`); 4 event — `exam_assigned` (emit di publish + tambah peserta saat tayang), `exam_opening`/`exam_closing` (cron `POST /api/internal/jobs/dispatch-reminders`, window H-1, closing hanya yg belum submit), `result_ready` (di `submit_attempt` non-pending + grading complete). API `/api/notifications` (list/unread-count/read/read-all); UI nav "Notifikasi" ber-badge di Sidebar + halaman `NotificationsPage` (poll ~45s). **Fase 2 = email via Resend** (butuh API key + verifikasi domain) + event tambahan opsional. |

| **M7** | **Layar pra-ujian** (instruksi, durasi, jumlah soal, **pakta integritas**) + **ketahanan koneksi** (indikator tersimpan, auto-resume) + **aksesibilitas** (ukuran font/kontras). | — | ✅ **SELESAI** (2026-08-05, tanpa migrasi). **M7.1** gerbang `PreExamGate` (info + instruksi dari `description` + pakta integritas) sebelum `start()`; resume lewati pakta; backend `GET /api/my-exams/{id}/intro`. **M7.2** hook `useAnswerSync` — antrean autosave (coalesce) + retry backoff + flush saat `online` + cadangan localStorage; indikator Tersimpan/Menyimpan/Gagal/Luring + banner koneksi. **M7.3** aksesibilitas di runner — skala teks A−/A/A+ (zoom pane, 4 tingkat) + toggle kontras tinggi (`.exam-hc` di globals.css), preferensi `ln_exam_prefs` tersimpan. |
| **M8** | **Anti-cheat**: deteksi pindah-tab/blur, fullscreen lock, cegah copy-paste, satu-sesi-aktif, log ke `audit_events`, (opsional) proctoring. (Acak urutan soal/opsi per-peserta DICORET 2026-08-01 — lihat §Security.) | F1, F2 | ✅ **M8 SELESAI (M8.1–M8.4)** (2026-08-06, migrasi 028): config `exams.anti_cheat` (JSONB, opt-in) + toggle di builder (StepDetail); tabel `attempt_events` + `exam_attempts.violation_count`; `POST /api/attempts/{id}/events` (batch, best-effort). Runner `useAntiCheat`: deteksi pindah-tab/blur, keluar fullscreen, blokir copy/paste/klik-kanan (seleksi boleh) → lapor + peringatan; **auto-submit setelah 1 peringatan** (config `on_focus_loss:submit`); overlay wajib fullscreen. PreExamGate: pengumuman langkah aktif + deteksi layar ganda (Window Management API). Admin: badge 'N pelanggaran' di daftar hasil + bagian Integritas (timeline) di review. **M8.3 ambang auto-submit SELESAI (violation_count=pelanggaran serius; input di builder). M8.4 kamera capture (mig 030, R2 folder proctor/, consent+retensi+galeri admin) SELESAI.** (Acak urutan soal DICORET 2026-08-01.) |
| **M9** | **Tipe soal non-MCQ + alur grading** (IELTS Writing/Speaking, matching, dst.). | F1 | ✅ **sebagian besar** via F1.1+F1.2 (sisa: speaking F1.3) |

Urutan default: **M1 → (F3/F4 secukupnya) → M2 → M3 → M4 → M5 → M6 → M7 → M8 → M9**. F1/F2 sudah disuntik sebagai sprint fondasi. **Selesai s/d M7 (penuh) + M6 Fase 1 + M8.1** (M3 & reminder M6 tinggal jadwalkan cron di deploy). **M8 anti-cheat SELESAI penuh (M8.1–M8.4). Kandidat berikutnya: F1.3 (speaking → audio ke R2), M6 Fase 2 (email/Resend), atau band F1.4a IELTS/iBT.**
...
- **F3.0** — Migrasi `015_foundation_audit.sql`: tabel `audit_events` (append-only) + `deleted_at` (exams/questions/question_passages) + `version` (exams) + `updated_by`. Service `audit_service.py`. _(user jalankan migrasi)_
